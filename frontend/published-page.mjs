...
export { extractPublishedMeaningFromDescription } from '../shared/published-content.mjs';
...
export function getPublishedContentSubLabel(record = {}, options = {}) {
  ...
  const contentSummary = String(record?.contentSummary || '').trim().slice(0, 240);
  if (contentSummary && !INTERNAL_WORKFLOW_COPY_PATTERN.test(contentSummary)) return contentSummary;
