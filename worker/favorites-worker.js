...
async function writeDailyRefreshFailure(env, error) {
  if (!env.FAVORITES) return;
  const today = dateKey(new Date());
  await env.FAVORITES.put(`daily-refresh:${today}`, JSON.stringify({
    dateKey: today,
    status: 'failed',
    startedAt: new Date().toISOString(),
    finishedAt: new Date().toISOString(),
    error: String(error || 'daily refresh worker configuration missing').slice(0, 500),
...
function cleanStoredData(data) {
  return stripInvalidCurrentTodaySnapshot(cleanStoredWorkflow(data));
}
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'POST') {
      ...
      const data = stripInvalidCurrentTodaySnapshot(cleanStoredWorkflow({
        ...current,
        words,
        statuses,
  ...
  async scheduled(controller, env, ctx) {
    ...
    if (siteUrl && autoRefreshSecret) {
      ...
      ctx.waitUntil(writeDailyRefreshFailure(
        env,
        !siteUrl
          ? 'Worker SITE_URL is not configured'
          : 'Worker AUTO_REFRESH_SECRET is not configured'
      ));
    }
